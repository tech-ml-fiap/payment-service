...
def generate_qr_data(order_id: int, amount: float) -> str:
    ...
    qr_data = (
        ...
        "020130"
        + transaction_uuid[:30]  # Identificador único
        + "040000"
        + order_id_str  # ID do pedido
        + "5303"
        + amount_str  # Valor do pedido
        + "5802BR"  # País (BR)
        + "5925"
        + client_str  # Nome do cliente
        + "6007"
        + city_str  # Cidade
        + "6207"
        + "0503***"  # Dados adicionais fixos
        + "6304"
        + "0B6D"  # Checksum fixo para mock
    )
    return qr_data
